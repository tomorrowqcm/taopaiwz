...
export default {
	...
	onLoad(options) {
		this.settingConfig = uni.getStorageSync('settingConfig');
		this.homeUrl = this.to_link("nft.taopainft.com/properties");

		// this.webviewList[this.WVindex].resume();
	},
	mounted() {
		this.Resource = [];
		this.top = uni.getSystemInfoSync().statusBarHeight + 0;
	...
	methods: {
		...
		createWebView(url) {
			...
			wv.evalJS("alert(\"仅供学习交流\");document.getElementById(\"__next\").style.backgroundColor=\"rgb(0 102 75 / 37%)\";");  
